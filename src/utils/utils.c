 ...
 * 
 * @param mode The current simulation snapshot mode.
 * @return 1 if secondary population is enabled, 0 otherwise.
 */
int isSecondaryPopulationEnabled(SnapshotMode mode) {
    // List here all modes that utilize the secondary (micron) dust population
    return (mode == SnapshotDriftTwoPop || mode == SnapshotGrowthTwoPop || mode == SnapshotDriftStaticGasTwoPop || mode == SnapshotGrowthStaticGasTwoPop);
}

/**
 * @brief Helper to determine if gas evolution is enabled in the current snapshot mode.
 * 
 * @param mode The current simulation snapshot mode.
 * @return 1 if gas evolution is enabled, 0 otherwise.
 */
int isGasEvolutionEnabled(SnapshotMode mode) {
    switch(mode) {
        ...
        case SnapshotDrift:
        case SnapshotGrowth:
        case SnapshotDriftTwoPop:
        case SnapshotGrowthTwoPop:
            return 1; // Gas evolution is enabled in these modes
        default:
            return 0; // Gas evolution is not enabled
    }
}
...
int isDustEnabled(SnapshotMode mode) {
    switch(mode) {
        ...
        case SnapshotDriftTwoPop:
        case SnapshotGrowthTwoPop:
        case SnapshotDriftStaticGas:
        case SnapshotGrowthStaticGas:
        case SnapshotDriftStaticGasTwoPop:
        case SnapshotGrowthStaticGasTwoPop:
            return 1; // Dust is enabled in these modes
        default:
            return 0; // Dust is not enabled
    }
}

int isDriftEnabled(SnapshotMode mode) {
    switch(mode) {
        case SnapshotDrift:
        case SnapshotDriftTwoPop:
        case SnapshotDriftStaticGas:
        case SnapshotDriftStaticGasTwoPop:
            return 1; // Drift is enabled in these modes
        default:
            return 0; // Drift is not enabled
    }
}
...
int isDustGrowthEnabled(SnapshotMode mode) {
    switch(mode) {
        ...
        case SnapshotGrowthStaticGas:
        case SnapshotGrowthStaticGasTwoPop:
            return 1; // Growth is enabled in these modes
        default:
            return 0; // Growth is not enabled
    }
}
...
/**
 * @brief Updates grid indices and mass arrays for both primary and secondary dust populations.
 * 
 * @param particle_data Pointer to the dust particle data structure.
 * @param current_time  The current simulation time.
 * @param particle_count Number of particles.
 * @param disk_params   Pointer to the disk parameters (grid info).
 * @param is_twopop     Flag indicating if the two-population model is enabled.
 */
void updateParticleGridIndices(ParticleData *particle_data, double current_time, int particle_count, 
                               const DiskParameters *disk_params, int is_twopop) {
    ...
    for (particle_index = 0; particle_index < particle_count; particle_index++) {
        ...
        if ((size_t)particle_count == particle_data->allocated_particle_number) {
            particle_data->dust_particle_mass_array[particle_index][0] = particle_data->dust_particle_mass_grid[particle_index];
        }
        particle_data->dust_particle_mass_array[particle_index][1] = grid_index;

        // Initialize mass at T=0
        if (current_time == 0) {
            particle_data->dust_particle_mass_array[particle_index][2] = particle_data->dust_particle_mass_array[particle_index][0];
        ...
        if (is_twopop && particle_data->micron_particle_distance_array != NULL) {
            ...
            // If T=0, initialize secondary mass too
            if (current_time == 0) {
                particle_data->micron_dust_particle_mass_array[particle_index][2] = particle_data->micron_dust_particle_mass_array[particle_index][0];
            }
        }
    }
}
