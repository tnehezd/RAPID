...
#include "io_utils.h"     // For timePar (though not called in tIntegrate, it's io-related), reszecskek_szama, ReadDustFile, Print_Sigma, Print_Pormozg_Size, Print_Mass, Print_Sigmad. Also for globals: filenev1, filenev3,
#include "disk_model.h"   // If any disk_model functions are called (e.g., calculate_boundary indirectly if sigma/press depend on it) - Though not directly visible in tIntegrate, often needed for global disk parameters. Add if you hit implicit declaration for disk_model functions.
#include "dust_physics.h" // For Count_Mass, secondaryGrowth, find_max, find_min, Get_Sigmad, Get_Radius
#include "utils.h"        // For time_step, get_gas_surface_density_pressure_pressure_gradient, and potentially other utility functions
...
void tIntegrate(disk_t *disk_params, const simulation_options_t *sim_opts, output_files_t *output_files) {
    ...
    if (sim_opts->drift == 1.) {
        ...
        // ReadDustFile hívása a részecskeadatok beolvasására
        ReadDustFile(p_data.radius, p_data.radiusmicr, p_data.massvec, p_data.massmicrvec, sim_opts->dust_input_filename);
    }
