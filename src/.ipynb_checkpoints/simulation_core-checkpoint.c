...
#include "disk_model.h"   // If any disk_model functions are called (e.g., Perem indirectly if sigma/press depend on Perem) - Though not directly visible in tIntegrate, often needed for global disk parameters. Add if you hit implicit declaration for disk_model functions.
...
void eqrhs(double pradius, double dp, double sigma, double ug, double r, double *drdt, const disk_t *disk_params) {
    ...
    *drdt = ug / (1. + St * St) + St / (1. + St * St) * H / P * dp * csound;    /* beáramlás sebessége: Birnstiel PHD    */
}
...
/* Mivel a különböző paramétereket csak a megadott gridcella pontokban ismerjük, de ez nem feltétlen egyezik meg a részecskék pozíciójával, ezért minden fontos paramétert interpolálunk a részecskék távolságára    */
    ...
    if(sim_opts->growth == 1.) {        // ha van részecskenövekedés
        if(time != 0.) {    // ha nem t0 időpontban vagyunk
            pradtemp = prad;
            interpol(disk_params->pressvec,disk_params->rvec,y,&p,disk_params->DD,opt,disk_params);
            pdens = disk_params->PDENSITY;    
            pradtemp = getSize(prad,pdens,sigma,sigmadd,y,p,dpress,step,disk_params);    // itt számolja a részecskenövekedést
            prad = pradtemp;
        }
    }
...
/* Itt számolja a részecske pozícióját    */
...
void tIntegrate(disk_t *disk_params, const simulation_options_t *sim_opts, output_files_t *output_files) {
    ...
    if (sim_opts->drift == 1.) {
        ...
        fprintf(stderr, "INFO [tIntegrate]: Particle drift is OFF. PARTICLE_NUMBER set to 0. Dust-related files will not be created.\n");
        PARTICLE_NUMBER = 0;
    }
    ...
    double masstempoinbuff = 0, masstempooutbuff = 0, massmtempoinbuff = 0, massmtempooutbuff = 0;
    double tavin = 0, tavout = 0, tavbuff = 0; // Távolságok a Print_Mass-hoz
    ...
    do {
        if (sim_opts->drift == 1.) {
            ...
            if (sim_opts->twopop == 1) {
                ...
                // de a meglévő find_min-t használva a reciprok trükkel:
            ...
            if ((fmod(current_time_years, (sim_opts->TMAX / sim_opts->WO)) < deltat || current_time_years == 0) && L - current_time_years < deltat) {
                ...
                // Ezeket a fájlneveket mindig előkészítjük, de az fopen-t feltételhez kötjük
                ...
                if(L != 0) { // Ne írjuk ki a fejléceket, ha L == 0, mert azt már a setup_initial_output_files megtette
                ...
                // --- MÓDOSÍTOTT RÉSZ: dust és micron_dust fájlok megnyitása ---
                if (sim_opts->growth == 1.0) {
                    output_files->dust_file = fopen(dust_name, "w");
                    if (output_files->dust_file == NULL) {
                        fprintf(stderr, "ERROR: Could not open %s for writing (growth is 1.0).\n", dust_name);
                    } else {
                        HeaderData_t dust_header_data = {.current_time = current_time_years, .is_initial_data = (current_time_years == 0.0)};
                        print_file_header(output_files->dust_file, FILE_TYPE_DUST_DENSITY, &dust_header_data); // Changed type
                    }

                    if (sim_opts->twopop == 1.) {
                        output_files->micron_dust_file = fopen(dust_name2, "w");
                        if (output_files->micron_dust_file == NULL) {
                            fprintf(stderr, "ERROR: Could not open %s for writing (growth is 1.0, twopop is 1.0).\n", dust_name2);
                        } else {
                            HeaderData_t micron_dust_header_data = {.current_time = current_time_years, .is_initial_data = (current_time_years == 0.0)};
                            print_file_header(output_files->micron_dust_file, FILE_TYPE_DUST_MICRON_DENSITY, &micron_dust_header_data); // Changed type
                        }
                    }
                } else {
                    // Ha a growth nincs bekapcsolva, biztosítsuk, hogy a fájlmutatók NULL-ok legyenek
                ...
                if (current_time_years == 0) {
                    ...
                    if (sim_opts->growth == 1.) { // Ez a hívás már feltételhez volt kötve
                ...
                if (sim_opts->evol == 1 || current_time_years == 0) {
                    if(output_files->surface_file != NULL) { // Ellenőrzés hozzáadva
                        Print_Sigma(disk_params, output_files);
                    } else {
                        fprintf(stderr, "WARNING: surface_file is NULL. Skipping Print_Sigma.\n");
                    }
                }

                // Particle position and size output
                if (sim_opts->drift == 1) { // Ez a blokk is már feltételhez volt kötve
                ...
                // ITT VOLT A PROBLEMATIKUS RÉSZ, AMELY VISSZAÁLLÍTOTTA A FLAG-EKET.
                // EZT A BLOKKOT KOMMENTELJÜK KI, HOGY A TÖMEG NE SZÁMOLÓDJON ÚJRA ÉS ÚJRA.
                /*
                if (sim_opts->dzone == 1.0 && PARTICLE_NUMBER > 0) {
                ...
                */
                ...
                Print_Mass(L, p_data.partmassind, p_data.partmassmicrind, t, masstempiin, masstempoin, massmtempiin, massmtempoin, &masstempiout, &masstempoout, &massmtempiout, &massmtempoout, masstempoinbuff, &masstempooutbuff, massmtempoinbuff, &massmtempooutbuff, &tavin, &tavout, &tavbuff, disk_params, sim_opts, output_files);
                ...
                masstempoinbuff = masstempooutbuff;
                massmtempoinbuff = massmtempooutbuff;
                ...
                // A close_snapshot_files függvény most már tartalmazza a growth ellenőrzést
